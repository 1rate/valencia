...
// Компонент ProductModal отображает подробную информацию о продукте в модальном окне
function ProductModal({ product, closeModal }) {
  const dispatch = useDispatch();

  // Получаем количество товара в корзине из состояния Redux
  const quantity = useSelector((state) => {
    const cartItem = state.cart.items.find((item) => item.id === product.id);
    return cartItem ? cartItem.quantity : 0;
  });

  // Мемоизируем обработчик добавления товара в корзину
  const handleAddToCart = useCallback(() => {
    dispatch({ type: 'ADD_TO_CART', payload: product });
  }, [dispatch, product]);

  // Мемоизируем обработчик увеличения количества товара
  const handleIncrease = useCallback(() => {
    dispatch({ type: 'ADD_TO_CART', payload: product });
  }, [dispatch, product]);

  // Мемоизируем обработчик уменьшения количества товара
  const handleDecrease = useCallback(() => {
    dispatch({ type: 'DECREASE_QUANTITY', payload: product });
  }, [dispatch, product]);

  // Мемоизируем обработчик остановки всплытия события
  const handleStopPropagation = useCallback((e) => {
    e.stopPropagation();
  }, []);

  // Мемоизируем обработчик нажатия клавиши Escape для закрытия модального окна
  ...
  // Блокируем прокрутку заднего плана и добавляем обработчик клавиши Escape при монтировании компонента
  useEffect(() => {
      ...
      >
        ...
        <div className="flex flex-col md:flex-row h-full">
          ...
          <div className="p-6 md:w-1/2 overflow-y-auto flex flex-col">
            ...
            {/* Блок управления количеством и добавления в корзину */}

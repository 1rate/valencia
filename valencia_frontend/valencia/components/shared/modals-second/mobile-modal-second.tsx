...
export const MobileProductModalSecond: React.FC<Props> = ({ product, className, onClose, onSubmit }) => {
  // Локальное состояние для управления открытием Drawer
  const [isOpen, setIsOpen] = useState(true);

  // Функция закрытия с задержкой, чтобы проигралась анимация
  const handleClose = () => {
    setIsOpen(false);
    // Допустим, анимация закрытия длится 300мс, после чего вызываем onClose
    setTimeout(() => {
      onClose();
    }, 300);
  };
    ...
    <Drawer open={isOpen} onOpenChange={handleClose}>
      <DrawerContent className={cn("w-full md:h-[70%] bg-white p-4 flex flex-col", className)}>
        ...
        <div className="mx-auto w-full max-w-sm">
          ...
          <DrawerFooter>
            <DrawerClose asChild>
              <Button onClick={() => onSubmit ? onSubmit() : handleClose()}>
